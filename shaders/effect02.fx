    ...
    float3 pos2 : TEXCOORD4) : COLOR
{
    ...
    // R.V^n    
    float4 specular = saturate(pow(saturate(dot(reflecto, view)), 2));
    
    float4 tuv = tex2D(TextureSample, uv);
    float chya = dot(normalize(light), normy) * 0.5 + 0.2;
...
{
    float4 Color = tex2D( g_scrSampler, Tex);  

    Color.a = 1.0f;
    Color.rgb = (Color.r+Color.g+Color.b)/3.0f;
     
    if (Color.r<0.1 || Color.r>0.9) Color.r = 0.0f; else Color.r = 1.0f;
    if (Color.g<0.1 || Color.g>0.9) Color.g = 0.0f; else Color.g = 1.0f;
    if (Color.b<0.1 || Color.b>0.9) Color.b = 0.0f; else Color.b = 1.0f;
...
float4 WithoutPS( float2 Tex : TEXCOORD0 ) : COLOR
{
    float4 Color = tex2D( g_scrSampler, Tex);  
    return Color;
}
...
technique RenderWithout
{
    pass p0
    {
        VertexShader = null;
        PixelShader = compile ps_2_0 WithoutPS();
        ZEnable = false;
    }
}
